...
const dialTab=$("dialTab"),labTab=$("labTab"),dialView=$("dialView"),labView=$("labView"),startExerciseButton=$("startExerciseButton"),bpmInput=$("bpmInput"),phraseSelect=$("phraseSelect"),difficultySelect=$("difficultySelect"),metronomeToggle=$("metronomeToggle"),targetAudioToggle=$("targetAudioToggle"),attemptLabel=$("attemptLabel"),beatLabel=$("beatLabel"),pitchScoreEl=$("pitchScore"),timingScoreEl=$("timingScore"),coverageScoreEl=$("coverageScore"),diagnosticBody=$("diagnosticBody"),fairnessPanel=$("fairnessPanel"),feedbackStatus=$("feedbackStatus"),exportButton=$("exportButton");
...
const POLICIES={
 relaxed:{pitchExcellent:40,pitchAcceptable:100,timingExcellent:120,timingAcceptable:260},
 standard:{pitchExcellent:20,pitchAcceptable:50,timingExcellent:70,timingAcceptable:170},
 precise:{pitchExcellent:10,pitchAcceptable:25,timingExcellent:45,timingAcceptable:100},
 virtuoso:{pitchExcellent:5,pitchAcceptable:12,timingExcellent:25,timingAcceptable:60}
};
...
let stableMidi=null,stableFrequency=null,currentConfidence=0,pendingJumpMidi=null,pendingJumpCount=0;
...
function analyzePitch(now){
 if(!analyser||!audioContext||now-lastAnalysisTime<ANALYSIS_INTERVAL_MS)return;lastAnalysisTime=now;analyser.getFloatTimeDomainData(timeDomainBuffer);const r=detectPitchYIN(timeDomainBuffer,audioContext.sampleRate);
 if(!r){currentConfidence*=.82;if(currentConfidence<.1)stableFrequency=null;captureSample(null);return}
 const rawMidi=frequencyToMidi(r.frequency);currentConfidence=.55*currentConfidence+.45*r.confidence;if(r.confidence>=MIN_CONFIDENCE){updateStablePitch(rawMidi,r.confidence);stableFrequency=midiToFrequency(stableMidi)}captureSample({rawMidi,stableMidi,confidence:r.confidence,rms:r.rms});
}
function captureSample(measurement){
 if(!exerciseRunning||exercisePhase!=="attempt"||!audioContext)return;const beat=(audioContext.currentTime-attemptStartTime)/secondsPerBeat();if(beat<0||beat>=EXERCISE_BEATS)return;
 if(measurement&&measurement.confidence>=MIN_CONFIDENCE&&measurement.stableMidi!=null)currentTrace.push({beat,...measurement});else currentTrace.push({beat,rawMidi:null,stableMidi:null,confidence:measurement?.confidence||0,rms:measurement?.rms||0});
}
...
function analyzeAttempt(){
 ...
 const note1=currentTrace.filter(p=>p.stableMidi!=null&&p.beat>=.20&&p.beat<=.82).map(p=>p.stableMidi);const referenceMidi=median(note1);
 if(referenceMidi==null)return{valid:false,reason:"No stable reference pitch was detected in note 1.",trace:[...currentTrace]};
 const normalized=currentTrace.map(p=>p.stableMidi==null?{...p,relative:null,rawRelative:null}:{...p,relative:p.stableMidi-referenceMidi,rawRelative:p.rawMidi-referenceMidi});
 const notes=[];
 for(let i=0;i<4;i++){
   const target=intervals[i],central=normalized.filter(p=>p.relative!=null&&p.beat>=i+.18&&p.beat<=i+.82),allFrames=normalized.filter(p=>p.beat>=i&&p.beat<i+1),folded=central.map(p=>octaveFoldSemitones(p.relative,target)),med=median(folded),pitchError=med==null?null:(med-target)*100,stability=med==null?null:stddev(folded.map(v=>(v-med)*100));
   const pitchTol=policy().pitchAcceptable/100;const correct=allFrames.filter(p=>p.relative!=null&&Math.abs(octaveFoldSemitones(p.relative,target)-target)<=pitchTol);const voiced=allFrames.filter(p=>p.relative!=null);const coverage=allFrames.length?voiced.length/allFrames.length:0;const correctCoverage=allFrames.length?correct.length/allFrames.length:0;
   let timingError=null;if(i===0||intervals[i]===intervals[i-1]){timingError=0}else{const search=normalized.filter(p=>p.relative!=null&&p.beat>=i-.35&&p.beat<=i+.45&&Math.abs(octaveFoldSemitones(p.relative,target)-target)<=pitchTol);if(search.length)timingError=(search[0].beat-i)*spb*1000}
   const pitchScore=pitchError==null?0:scoreFromError(pitchError,policy().pitchExcellent,policy().pitchAcceptable);const timingScore=timingError==null?0:(i===0||intervals[i]===intervals[i-1]?Math.round(correctCoverage*100):scoreFromError(timingError,policy().timingExcellent,policy().timingAcceptable));
   notes.push({index:i+1,targetSemitones:target,pitchErrorCents:pitchError,timingErrorMs:timingError,coverage,correctCoverage,stabilityCents:stability,pitchScore,timingScore,samples:central.length});
 }
 return{valid:true,phrase:phrase().name,phraseKey:phraseSelect.value,bpm:Number(bpmInput.value),difficulty:difficultySelect.value,policy:{...policy()},referenceMidi,referenceNote:midiToNoteName(referenceMidi),notes,pitchScore:Math.round(mean(notes.map(n=>n.pitchScore))),timingScore:Math.round(mean(notes.map(n=>n.timingScore))),coverage:Math.round(100*mean(notes.map(n=>n.coverage))),trace:normalized,timestamp:new Date().toISOString(),fairness:null};
}
function fmtSigned(v,unit){if(v==null)return"—";return`${v>=0?"+":""}${Math.round(v)}${unit}`}
function renderResult(result){lastResult=result;fairnessPanel.classList.remove("hidden");fairnessButtons.forEach(b=>b.classList.remove("selected"));feedbackStatus.textContent="";if(!result.valid){pitchScoreEl.textContent="—";timingScoreEl.textContent="—";coverageScoreEl.textContent="—";diagnosticBody.innerHTML=`<tr><td colspan="6">${result.reason}</td></tr>`;statusEl.textContent=result.reason;drawTraceGraph();return}pitchScoreEl.textContent=result.pitchScore;timingScoreEl.textContent=result.timingScore;coverageScoreEl.textContent=`${result.coverage}%`;diagnosticBody.innerHTML=result.notes.map(n=>`<tr><td>${n.index}</td><td>${n.targetSemitones>=0?"+":""}${n.targetSemitones} st</td><td>${fmtSigned(n.pitchErrorCents,"¢")}</td><td>${n.index===1||phrase().intervals[n.index-1]===phrase().intervals[n.index-2]?"occupancy":fmtSigned(n.timingErrorMs," ms")}</td><td>${Math.round(n.coverage*100)}%</td><td>${n.stabilityCents==null?"—":Math.round(n.stabilityCents)+"¢ SD"}</td></tr>`).join("");statusEl.textContent=`Measured: pitch ${result.pitchScore}, timing ${result.timingScore}. Rate whether that feels fair.`;drawTraceGraph()}

function drawTraceGraph(){const w=traceCanvas.clientWidth,h=traceCanvas.clientHeight;if(!w||!h)return;tctx.clearRect(0,0,w,h);tctx.fillStyle="#121212";tctx.fillRect(0,0,w,h);const intervals=phrase().intervals,pad={l:42,r:14,t:18,b:30},pw=w-pad.l-pad.r,ph=h-pad.t-pad.b,vals=[...intervals];const trace=lastResult?.trace||currentTrace.map(p=>({beat:p.beat,relative:null,rawRelative:null}));for(const p of trace)if(p.relative!=null)vals.push(p.relative);let lo=Math.floor(Math.min(-2,...vals)/2)*2-2,hi=Math.ceil(Math.max(12,...vals)/2)*2+2;if(hi-lo<12)hi=lo+12;const x=b=>pad.l+pw*b/4,y=s=>pad.t+ph*(hi-s)/(hi-lo);tctx.font="12px system-ui";for(let s=Math.ceil(lo/2)*2;s<=hi;s+=2){const yy=y(s);tctx.strokeStyle=s===0?"#555":"#292929";tctx.beginPath();tctx.moveTo(pad.l,yy);tctx.lineTo(w-pad.r,yy);tctx.stroke();tctx.fillStyle="#888";tctx.textAlign="right";tctx.textBaseline="middle";tctx.fillText(`${s>0?"+":""}${s}`,pad.l-7,yy)}for(let b=0;b<=4;b++){const xx=x(b);tctx.strokeStyle="#393939";tctx.beginPath();tctx.moveTo(xx,pad.t);tctx.lineTo(xx,h-pad.b);tctx.stroke();if(b<4){tctx.fillStyle="#999";tctx.textAlign="center";tctx.textBaseline="top";tctx.fillText(`${b+1}`,x(b+.5),h-pad.b+7)}}tctx.strokeStyle="#bfbfbf";tctx.lineWidth=3;tctx.setLineDash([7,6]);for(let i=0;i<4;i++){tctx.beginPath();tctx.moveTo(x(i+.06),y(intervals[i]));tctx.lineTo(x(i+.94),y(intervals[i]));tctx.stroke()}tctx.setLineDash([]);
 if(lastResult?.valid){drawSeries(lastResult.trace.filter(p=>p.rawRelative!=null).map(p=>({beat:p.beat,v:p.rawRelative})),x,y,"#777",1,[2,4]);drawSeries(lastResult.trace.filter(p=>p.relative!=null).map(p=>({beat:p.beat,v:p.relative})),x,y,"#f4f4f4",2.5,[])}
...
resizeCanvases();drawPitchCircle();drawTraceGraph();
